...
namespace the_Cryptocurrencies_bot
{
    public class API
    {
        static async Task Main(string[] args)
        {
            ...
            var chatID = "@CryptosWorldTrading2";
            ...
            for (int i = 0; i < usdt.Count; i++)
            {

                string empty = string.Empty;

                Console.WriteLine($"{i}     {usdt[i]}");

                string symbol2 = usdt[i];
                string price2 = priceUsdt[i];
                string change2 = changeUsdt[i];
                string low2 = lowUsdt[i];
                string high2 = highUsdt[i];


                if (symbol2[symbol2.Length - 1] == 'T')
                {
                    List<string> cryptoName = new List<string>();

                    z = decimal.Parse(change2);
                    ...
                    conn.Close();
                            ...
                            symbol2 == "SYLO-USDT" || symbol2 == "POLK-USDT" || symbol2 == "BOLT-USDT" || symbol2 == "CAS-USDT")
                        {
                            try
                            {
                                conn.Open();
                                m = String.Empty;
                                sql = $"SELECT * from Main where crypto = '{symbol2}';";
                                ...
                                if (z >= 10 && z < 20)
                                {
                                    if (plus101 == 0)
                                    {
                                        await abc.SendMessage10PAsync(chatID, symbol2,
                                            price2, xyz, low2, high2);

                                        conn2.Open();
                                        m = String.Empty;
                                        sql = $"UPDATE Main SET plusdiez = 1 where crypto = '{symbol2}';";
                                ...
                                else if (z <= -10 && z > -20)
                                {
                                    if (minus101 == 0)
                                    {
                                        await abc.SendMessage10NAsync(chatID, symbol2,
                                            price2, xyz, low2, high2);

                                        conn2.Open();
                                        m = String.Empty;
                                        sql = $"UPDATE Main SET mainnesdiez = 1 where crypto = '{symbol2}';";
                                        cmd = new MySqlCommand(sql, conn2);
                                        var rdr2 = cmd.ExecuteReader();
                                        rdr2.Close();
                                        conn2.Close();
                                ...
                                else if (z >= 20 && z < 30)
                                {
                                    if (plus201 == 0)
                                    {
                                        await abc.SendMessage20PAsync(chatID, symbol2,
                                            price2, xyz, low2, high2);

                                        conn2.Open();
                                        m = String.Empty;
                                        sql = $"UPDATE Main SET plusveinte = 1 where crypto = '{symbol2}';";
                                        cmd = new MySqlCommand(sql, conn2);
                                        var rdr2 = cmd.ExecuteReader();
                                        rdr2.Close();
                                        conn2.Close();
                                ...
                                else if (z <= -20 && z > -30)
                                {
                                    if (minus201 == 0)
                                    {
                                        await abc.SendMessage20NAsync(chatID, symbol2,
                                            price2, xyz, low2, high2);

                                        conn2.Open();
                                        m = String.Empty;
                                        sql = $"UPDATE Main SET mainnesveinte = 1 where crypto = '{symbol2}';";
                                        cmd = new MySqlCommand(sql, conn2);
                                        var rdr2 = cmd.ExecuteReader();
                                        rdr2.Close();
                                        conn2.Close();
                                ...
                                else if (z >= 30)
                                {
                                    if (plus301 == 0)
                                    {
                                        await abc.SendMessage30PAsync(chatID, symbol2,
                                            price2, xyz, low2, high2);

                                        conn2.Open();
                                        m = String.Empty;
                                        sql = $"UPDATE Main SET plustreinta = 1 where crypto = '{symbol2}';";
                                        cmd = new MySqlCommand(sql, conn2);
                                        var rdr2 = cmd.ExecuteReader();
                                        rdr2.Close();
                                        conn2.Close();
                                ...
                                else if (z <= -30)
                                {
                                    if (minus301 == 0)
                                    {
                                        await abc.SendMessage30NAsync(chatID, symbol2,
                                           price2, xyz, low2, high2);

                                        conn2.Open();
                                        m = String.Empty;
                                        sql = $"UPDATE Main SET mainnestreinta = 1 where crypto = '{symbol2}';";
                                        cmd = new MySqlCommand(sql, conn2);
                                        var rdr2 = cmd.ExecuteReader();
                                        rdr2.Close();
                                        conn2.Close();
